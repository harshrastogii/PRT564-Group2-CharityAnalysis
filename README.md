...
Regression analysis of the Australian Charities and Not-for-profits Commission (ACNC) Charity Register, integrated with the 2023 Annual Information Statement (AIS), investigating which operational and geographic factors predict charity governance capacity — with a particular focus on the Northern Territory.
...
|---------|---------|-----------|---------|
| ACNC Charity Register | 65,114 | 69 | Primary dataset — operational attributes, purposes, beneficiary groups, jurisdictions, governance structure |
| ACNC 2023 Annual Information Statement (AIS) | 53,323 | 91 | Supplementary dataset — financial variables (Total Revenue, Total Expenses, Total Assets, Net Assets) and staffing (full-time, part-time, volunteers) |

**Join strategy:** Left join on Australian Business Number (ABN). Left join was chosen deliberately over inner join to preserve charities without AIS returns, since smaller and newer charities are systematically more likely to miss AIS filings. An inner join would have biased the sample toward larger, better-resourced organisations. **Match rate: 77.9%** — 50,700 of 65,114 register charities have matching AIS financial data.
...
Raw Register (65,114)
    ↓
AIS 2023 join on ABN (left) → +7 financial and staffing variables
    ↓                         (77.9% match rate)
State standardisation       → 58,235 valid state records
    ↓
Binary encoding (47 cols)   → Y/blank → 1/0
    ↓
Feature engineering         → +5 derived features
    ↓
Target cleaning             → Remove zero-target + top 1% outliers
    ↓
...
| `Is_NT` | Binary flag for charities headquartered in the Northern Territory |
| `Size_Encoded` | Ordinal encoding of charity size (Small=1, Medium=2, Large=3) |
...
| Independent t-test: NT vs Non-NT charities (n=564 vs n=57,283) | t = 5.008 | < 0.001 | NT charities have significantly more responsible persons (5.73 vs 5.11) |
...
The register does not describe a uniform sector. Northern Territory has 867 registered charities in total — 1.5% of the national sector — with 76% serving Aboriginal and Torres Strait Islander communities. Within our regression sample, the 564 NT-headquartered charities have statistically significantly larger governance boards than the national average, and this effect persists after controlling for charity size. This is not a compliance burden to be reduced; it reflects cultural governance structures that require broader community representation. A single national compliance framework systematically misfits remote and Indigenous-led organisations.
...
Open `regression_analysis.ipynb` in Jupyter and run all cells. The notebook outputs model coefficients, evaluation metrics, statistical test results, and saves all evaluation charts.
...
├── assignment1/
│   └── notebooks/                      # Assessment 1 EDA work
├── assignment2/
│   └── regression_analysis.ipynb       # Main regression pipeline (Assessment 2)
├── assets/
│   └── charts/                         # EDA and evaluation charts
└── data/
    └── (raw data not committed — download from data.gov.au)
```
...
- **AIS financial variables were integrated but not included in the regression feature set.** This was a deliberate methodological choice: including them would have required dropping approximately 14,000 records without AIS returns, systematically biasing the sample toward larger, better-resourced charities. The join is structurally in place for future analyses.
- **NT sub-sample is small** (n = 564 in the regression, 867 in the full register), which limits the statistical power of NT-specific sub-analyses.

---

## Future Work

1. Extend the regression feature set with AIS financial variables using a multiple-imputation or separate-model approach that accommodates partial coverage.
2. Fit a Poisson or negative binomial regression to test whether the distributional mismatch changes the coefficient structure.
3. Run an `Is_NT × Size_Encoded` interaction model to isolate the NT effect from pure size effects.
4. Qualitative validation with Aboriginal-controlled organisations to test the cultural-governance interpretation before any policy recommendation is operationalised.
